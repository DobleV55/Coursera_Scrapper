...
download_links_titles = []
f = open("coursera_links", "a")
fil = open("coursera_titles", "a")
all_videos = open("all_videos.txt").readlines()
...
for line in all_videos:
    ...
    line = dict(line)
    for item in line:
        if 'lecture' in line[item]:
            link = line[item]
            driver.get(link)
            time.sleep(10)
            if driver.find_element_by_class_name('rc-DownloadsDropdown'):
                download = driver.find_element_by_class_name('rc-DownloadsDropdown')
            else:
                time.sleep(5)
                download = driver.find_element_by_class_name('rc-DownloadsDropdown')
            button = download.find_element_by_class_name('bt3-dropdown-toggle')
            button.click()
            ul_tag = download.find_element_by_class_name('bt3-dropdown-menu')
            a_tag = ul_tag.find_element_by_tag_name('a')
            mp4_link = (a_tag.get_attribute('href'))
            f.write(mp4_link+'\n')
            fil.write(item+'\n')
            print(item)
f.close()
